if CR == nil then
  CR = {
    ...
    -- Pot configurations
    house_seed = 1000,      -- The baseline buy-in from the house
    total_pot = 1000,       -- Running total (starts at house_seed)
    ...
    chat_templates = {
      bet = "<player> backs <result> (#<total>) with <bet> gil.",
...
local function generate_field()
  ...
  CR.bets = {}
  CR.last_segment = 0
  CR.total_pot = CR.house_seed -- Reset pot back to house baseline
...
local function announce_betting_board()
  ...
  for i = 1, #CR.field do
    local r = CR.field[i]
    table_announce("#" .. tostring(r.number) .. " " .. r.name .. " | " .. (r.style_tag or ""))
  end
end
...
local function place_bet_for_player(name, pick, amount)
  ...
  -- Recalculate total running pot (House Seed + sum of all active wagers)
  local running_wagers = 0
  for p_name, _ in pairs(CR.bets) do
    running_wagers = running_wagers + (dealer_get_wager(p_name) or 0)
  end
  CR.total_pot = CR.house_seed + running_wagers
...
local function finish_race()
  ...
  local total_winning_wagers = 0

  -- First Pass: Calculate total pool money riding on the WINNING bird
  if next(CR.bets) ~= nil then
    for name, pickNum in pairs(CR.bets) do
      if pickNum == winner.number then
        total_winning_wagers = total_winning_wagers + math.max(0, dealer_get_wager(name))
      end
    end
  end

  -- Second Pass: Process payments/deductions
  if next(CR.bets) ~= nil then
    for name, pickNum in pairs(CR.bets) do
      local wager = math.max(0, dealer_get_wager(name))
      local delta = 0

      if pickNum == winner.number then
        -- Winner: Proportional slice of the entire total pot
        local share_ratio = wager / total_winning_wagers
        local total_payout = math.floor(share_ratio * CR.total_pot)
        
        -- Net winnings is their total payout minus their original stake
        delta = total_payout - wager 
        
        dealer_add_bank(name, delta)
        table.insert(winning_players, name .. ", winning " .. tostring(total_payout) .. " gil!")
      else
        -- Loser: Forfeits their wager to the pot
        delta = -wager
        dealer_add_bank(name, delta)
      end
    end
  ...
  -- Handle the dynamic single winner / multiple winners announcement formatting
  if #winning_players > 1 then
    table_announce("Winners splitting the pot: " .. table.concat(winning_players, ", "))
  elseif #winning_players == 1 then
    table_announce("Winner: " .. winning_players[1])
  else
    table_announce("No winners this race. The House claims the pot!")
  end
...
function draw_config_ui()
  ...
  CR.house_seed = ui_input_int("House Pot Seed (gil)", tonumber(CR.house_seed) or 1000)
...
function draw_ui()
  ...
  ui_separator()
  ui_text_colored("Current Total Pot: " .. tostring(CR.total_pot) .. " gil (House Seed: " .. tostring(CR.house_seed) .. ")", 1.0, 0.8, 0.2, 1.0)
  ui_separator()
  ...
  for i = 1, #CR.field do
    ...
    -- Calculate total gil currently riding on this bird
    local bird_gil = 0
    for p_name, p_pick in pairs(CR.bets) do
      if p_pick == r.number then bird_gil = bird_gil + (dealer_get_wager(p_name) or 0) end
    end

    ui_text(
      "#" .. tostring(r.number) .. " " .. r.name
      .. " | Pooled Bet Gil: " .. tostring(bird_gil)
  ...
  ui_separator()
  ui_text_colored("Active Bets Placed", 1.0, 1.0, 0.8, 1.0)
  ...
  for name, pick in pairs(CR.bets) do
    ...
    ui_text(name .. " -> #" .. tostring(pick) .. " " .. racerName .. " (Wager: " .. tostring(dealer_get_wager(name)) .. " gil)")
  end
